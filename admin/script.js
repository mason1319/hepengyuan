...
function updateFileState() {
  ...
  if (file && !form.elements.slug.value) {
    ...
    form.elements.slug.dataset.autoGenerated = "true";
  }
}

function makeRetrySlug(value) {
  const base = String(value || "media").replace(/-+$/, "").slice(0, 82) || "media";
...
form.addEventListener("input", (event) => {
  ...
  if (control === form.elements.slug) control.dataset.autoGenerated = "false";
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    let metadata = uploadPayload(publicFile, durationSeconds);
    try {
      await uploadFile(publicFile, metadata, posterBlob);
    } catch (error) {
      const slugField = form.elements.slug;
      const canRetryWithGeneratedSlug = error.status === 409 && /slug/.test(error.message) && slugField.dataset.autoGenerated === "true";
      if (!canRetryWithGeneratedSlug) throw error;
      slugField.value = makeRetrySlug(slugField.value);
      metadata = uploadPayload(publicFile, durationSeconds);
      setStatus(formStatus, `原 slug 已存在，已自动改为“${slugField.value}”，正在重试上传…`);
      await uploadFile(publicFile, metadata, posterBlob);
    }
